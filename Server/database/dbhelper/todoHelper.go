...
func ToggleTodoStatus(todoID int, userID int) error {
	// Toggle status using SQL: status = NOT status
	_, err := db.DB.Exec(`
		UPDATE todos
		SET status = NOT status, updated_at = $3
...
func GetTodosStatus(userID int, status string) ([]models.Todo, error) {
	...
	// Normalize status query parameter
	switch status {
	case "completed", "true":
	...
	default:
		// Return all tasks if status is empty or unknown
		err = db.DB.Select(&todos, `
			SELECT id, title, description, status, created_at, updated_at
			FROM todos
